...
void SDN_switch::recv_handler (packet *p){
    ...
    if (p->type() == "SDN_data_packet" /*&& !hi*/ ) { // the switch receives a packet from the other switch
        ...
        p2->getHeader()->setNexID ( nextID[p->getHeader()->getDstID()] ); //設定send的目標
        p2->getHeader()->setDstID ( p->getHeader()->getDstID() ); //設定終點
        //hi = true;
...
///////////////////////////////////////////////////
    ...
    for(int i = 0; i < inputs.size(); i++)      //sorted
    {
        ...
        {
            ctrl_packet_event(inputs[i].id, inputs[i].time, inputs[i].msg);
            //cout << "ctrl " << "time: " << inputs[i].time << " id:" << inputs[i].id << " msg: " << inputs[i].msg << " packet id: " << i << endl;
        }
        else
        {
            data_packet_event(inputs[i].src, inputs[i].dst, inputs[i].time);
            //cout << "data " << "time: " << inputs[i].time << " src: " << inputs[i].src << " data: " << inputs[i].dst << " packet id: " << i << endl;
        }
    }
    // start simulation!!
