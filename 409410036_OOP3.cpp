...
void SDN_controller::recv_handler(packet *p)
{
    ...
    if (p->type() == "SDN_ctrl_packet") 
    {
        ...
        else //ack
        {
            ...
            for(int i = 0; i < Nodes; i++)
            {
                if((pass[mat][i] == 1 || pass[mat][i] == 2) && nodes[i].order[mat] < ROP)
                    ROP = nodes[i].order[mat];
            }
            
            for(int i = 0; i < Nodes; i++)
            {
                if(pass[mat][i] == 1 && nodes[i].order[mat] == ROP)
                {
                    pass[mat][i] = 2;
                    ctrl_packet_event(Nodes, i, mat, tmp_act[mat][i], event::getCurTime(), "start");
                }
...
void shortestPath(vector<int> &dists, vector<int> &visited, int min, const int dst, int times, const int flag)
{
    ...
    for (int i = 0; i < Nodes; i++)
    {
        if (flag == 0)
        {
            if (dists[min] >= 1000000000 || nodes[min].oldResult.count(dst) == 1) //路徑無限大 || 此點已有最短路徑
                min = i;
        ...
        {
            if (dists[min] >= 1000000000 || nodes[min].newResult.count(dst) == 1)
                min = i;
...
bool cmp(const inputEevent &a, const inputEevent &b)
{
    if (a.time < b.time)
        return true;
    else if (a.time > b.time)
        return false;
    if (a.ctrlOrData < b.ctrlOrData)
        return true;
    else if (a.ctrlOrData > b.ctrlOrData)
        return false;
    if (a.ctrlOrData == 0)
    {
        if (a.id < b.id)
...
int main() 
{
    ...
    for (int i = 0; i < Nodes; i++)
    {
        node::id_to_node(i)->add_phy_neighbor(Nodes);
        node::id_to_node(Nodes)->add_phy_neighbor(i);
    }
    ...
    for (int i = 0; i < Links; i++) // input link
    {
        ...
        links.push_back(tmp);
        node::id_to_node(tmp.Node1)->add_phy_neighbor(tmp.Node2);
        node::id_to_node(tmp.Node2)->add_phy_neighbor(tmp.Node1);
    }
    ...
    for(int i = 0; i < Dsts; i++)   //initialize order
    {
        ...
        {
            nodes[j].order[dsts[i]] = -1;
            if(dsts[i] == j)
                nodes[j].order[dsts[i]] = 0;
        }
    }
    for(int i = 0; i < Dsts; i++)   //calculate order (level order)
    {
        ...
        do
        {
            for(int j = 0; j < Nodes; j++)  //計算新路徑與爭點的距離
            {
                if(j == dsts[i]) continue;
                if(nodes[nodes[j].newResult[dsts[i]]].order[dsts[i]] == order-1 )
                    nodes[j].order[dsts[i]] = order;
            }
            int flag = 0;
            for(int j = 0; j < Nodes; j++)  
            {
                if(nodes[j].order[dsts[i]] == -1)   //尚未完成
                {
                    flag = 1;   //繼續do while
                    break;
                }
        ...
        {
            if(nodes[j].oldResult[dsts[i]] != nodes[j].newResult[dsts[i]])
                if(min > nodes[j].order[dsts[i]])
                    min = nodes[j].order[dsts[i]];
        }
        for(int j = 0; j < Nodes; j++)      //將其校正為0 方便controller判斷
        {
            nodes[j].order[dsts[i]] -= min; 
        }
    }
    ...
    for (unsigned int id = 0; id < Nodes; id++) // input oldResult
    {
        for (int j = 0; j < Dsts; j++)
        {
            ...
            tmp.msg = "old";
            tmp.mat = dsts[j];
            tmp.act = nodes[id].oldResult[dsts[j]];
    ...
    for (unsigned int id = 0; id < Nodes; id++) // input newResult
    {
        for (int j = 0; j < Dsts; j++)
        {
            if (id == nodes[id].newResult[dsts[j]] || nodes[id].newResult[dsts[j]] == nodes[id].oldResult[dsts[j]])
                continue;
            ...
            tmp.msg = "new";
            tmp.mat = dsts[j];
            tmp.act = nodes[id].newResult[dsts[j]];
            inputs.push_back(tmp);
        }
    }
    ...
    //sort(inputs.begin(), inputs.end(), cmp); // sort events
    for (int i = 0; i < inputs.size(); i++)  // sorted
    {
        if (inputs[i].ctrlOrData == 0)
            ctrl_packet_event(Nodes ,inputs[i].id, inputs[i].mat, inputs[i].act, inputs[i].time, inputs[i].msg);
        else
            data_packet_event(inputs[i].src, inputs[i].dst, inputs[i].time);
    }
