...
#define Get(func_name, type, var_name) \
    type func_name() { return var_name; }
///////////////////////////////////////////////////////////////////////////////////////
int Time = 0, srcID = 0, dstID = 0;
///////////////////////////////////////////////////////////////////////////////////////
class inputEevent
{
public:
...
public:
    ...
    // SET(setActID, unsigned int, actID, _actID);
    // GET(getActID, unsigned int, actID);
    // GET(getWeight, int, weight);
    // SET(setWeight, int, weight, _weight);
    Get(getlinktable, vector<Link>, linktable);
    SET(setlinktable, vector<Link>, linktable, _linktable);
...
class SDN_controller : public node
{
    ...
    vector<vector<int>> tmp_act;    //記錄尚不可更新者的下一家
    vector<vector<int>> pass;       //更新進度
    map<int,int> stat;
    vector<Link> linktable;
    vector<Node> nodetable;

    void mystart_shortestPath(int, const int);
    void myshortestPath(vector<int> &dists, vector<int> &visited, int min, const int dst, int times, const int flag);
    void cal_order();
...
void SDN_controller::myshortestPath(vector<int> &dists, vector<int> &visited, int min, const int dst, int times, const int flag)
{

    if (flag == 0) //距離最短者(min)的下家 記錄到result
        nodetable[min].oldResult[dst] = visited[min];
    else
        nodetable[min].newResult[dst] = visited[min];
    ...
    for (int i = 0; i < 2*Links; i++)
    {
        int tmp;
        //尋找與min有關的link
        if (linktable[i].Node1 == min)
            tmp = linktable[i].Node2;

        else if (linktable[i].Node2 == min)
            tmp = linktable[i].Node1;

        else
            continue;

        //此link上的另一個node(tmp)已經找到最短路徑
        if (flag == 0 && nodetable[tmp].oldResult.count(dst) == 1)
            continue;
        else if (flag == 1 && nodetable[tmp].newResult.count(dst) == 1)
            continue;

        //計算經過此link的最短路徑
        int weight = flag ? linktable[i].newW : linktable[i].oldW;
        if (dists[tmp] >= 1000000000 || dists[tmp] > dists[min] + weight) //第一次到tmp || 距離變短
        {
    ...
    for (int i = 0; i < Nodes; i++)
    {
        if (flag == 0)
        {
            if (dists[min] >= 1000000000 || nodetable[min].oldResult.count(dst) == 1) //路徑無限大 || 此點已有最短路徑
                min = i;

            else if (nodetable[i].oldResult.count(dst) == 0 && dists[i] < dists[min]) //尚未找到最短路徑 && 路徑較dists[min]短
                min = i;
        }
        else
        {
            if (dists[min] >= 1000000000 || nodetable[min].newResult.count(dst) == 1)
                min = i;

            else if (nodetable[i].newResult.count(dst) == 0 && dists[i] < dists[min])
                min = i;
        }
    }
    myshortestPath(dists, visited, min, dst, times - 1, flag);
}
void SDN_controller::mystart_shortestPath(int dst, const int flag)
{
    nodetable.resize(Nodes);
    for (int i = 0; i < Nodes; i++) // initialize
        nodetable[i].ID = i;
    vector<int> dists(Nodes, 1000000000); //與終點的距離
    vector<int> visited(Nodes, -1);       //初始化-1 記錄next node
    dists[dst] = 0;
    visited[dst] = dst; //終點下家
    myshortestPath(dists, visited, dst, dst, Nodes, flag);
    return;
}
void SDN_controller::cal_order()
{
    for(int i = 0; i < Dsts; i++)   //initialize order
    {
        for(int j = 0; j < Nodes; j++)
        {
            nodetable[j].order[dsts[i]] = -1;
            if(dsts[i] == j)
                nodetable[j].order[dsts[i]] = 0;
        }
    }
    for(int i = 0; i < Dsts; i++)   //calculate order (level order)
    {
        int order = 1;
        do
        {
            for(int j = 0; j < Nodes; j++)  //計算新路徑與終點的距離
            {
                if(j == dsts[i]) continue;
                if(nodetable[nodetable[j].newResult[dsts[i]]].order[dsts[i]] == order-1 )
                    nodetable[j].order[dsts[i]] = order;
            }
            int flag = 0;
            for(int j = 0; j < Nodes; j++)  
            {
                if(nodetable[j].order[dsts[i]] == -1)   //尚未完成
                {
                    flag = 1;   //繼續do while
                    break;
                }
        ...
        int min = 100000000;
        for(int j = 0; j < Nodes; j++)      //計算要更新的node中離終點最近者的距離
        {
            if(nodetable[j].oldResult[dsts[i]] != nodetable[j].newResult[dsts[i]])
                if(min > nodetable[j].order[dsts[i]])
                    min = nodetable[j].order[dsts[i]];
        }
        for(int j = 0; j < Nodes; j++)      //將其校正為0 方便controller判斷
        {
            nodetable[j].order[dsts[i]] -= min; 
        }
    }
}
...
void SDN_stat_pkt_gen_event::trigger()
{
    ...
    // payload
    pld->setMsg(msg);
    //pld->setWeight(weight);
    pld->setlinktable(linktable);
    //pld->setMatID(mat);
    //pld->setActID(act);
...
void stat_packet_event(unsigned int con_id, unsigned int id, vector<Link> linktable, unsigned int t = event::getCurTime(), string msg = "default")
{
    // void ctrl_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    if (id == BROCAST_ID || node::id_to_node(id) == nullptr)
    {
    ...
    e_data.src_id = src;
    e_data.dst_id = id;
    // e_data.weight = weight;
    e_data.linktable = linktable;
    //e_data.mat_id = mat;
    //e_data.act_id = act;
    e_data.msg = msg;
...
{
    if (p == nullptr || p->type() == "SDN_data_packet")
        return;
    if(p->type() == "SDN_stat_packet")
    {
        SDN_stat_packet *p1 = nullptr;
        p1 = dynamic_cast<SDN_stat_packet *>(p);
        SDN_stat_payload *l1 = nullptr;
        l1 = dynamic_cast<SDN_stat_payload *>(p1->getPayload());
        if(stat[p1->getHeader()->getPreID()] == 0)  //install
        {
            add_phy_neighbor(p1->getHeader()->getPreID());  //con_id
            stat[p1->getHeader()->getPreID()] = 1;

            for(int i = 0 ; i < l1->getlinktable().size(); i++)
                linktable.push_back(l1->getlinktable()[i]);

            for(int i = 0; i < getNodeID(); i++)    //check
                if(stat[i] == 0)
                    return;
            //cout <<"ALL Clear\n";

            for (int i = 0; i < Dsts; i++)
                mystart_shortestPath(dsts[i], 0); // oldW
            
            for (unsigned int id = 0; id < Nodes; id++) // input oldResult
            {
                for (int j = 0; j < Dsts; j++)
                {
                    if (id == nodetable[id].oldResult[dsts[j]])
                        continue;
                    //string msg = to_string(dsts[j]) + " " + to_string(nodes[id].oldResult[dsts[j]]);
                    inputEevent tmp;
                    tmp.id = id;
                    tmp.time = event::getCurTime();
                    tmp.msg = "old";
                    tmp.mat = dsts[j];
                    tmp.act = nodetable[id].oldResult[dsts[j]];
                    ctrl_packet_event(Nodes ,tmp.id, tmp.mat, tmp.act, tmp.time, tmp.msg);
                }
            }
            linktable.clear();
        }
        else if(stat[p1->getHeader()->getPreID()] == 1) //update
        {
            stat[p1->getHeader()->getPreID()] = 2;
            for(int i = 0 ; i < l1->getlinktable().size(); i++)
                linktable.push_back(l1->getlinktable()[i]);
            ...
            for (int i = 0; i < Dsts; i++)
                mystart_shortestPath(dsts[i], 1); // newW
            cal_order();

            for (unsigned int id = 0; id < Nodes; id++) // input newResult
            {
                for (int j = 0; j < Dsts; j++)
                {
                    if (id == nodetable[id].newResult[dsts[j]] || nodetable[id].newResult[dsts[j]] == nodetable[id].oldResult[dsts[j]])
                        continue;
                    //string msg = to_string(dsts[j]) + " " + to_string(nodes[id].newResult[dsts[j]]);
                    inputEevent tmp;
                    tmp.ctrlOrData = 0;
                    tmp.id = id;
                    tmp.time = event::getCurTime();
                    tmp.msg = "new";
                    tmp.mat = dsts[j];
                    tmp.act = nodetable[id].newResult[dsts[j]];
                    ctrl_packet_event(Nodes ,tmp.id, tmp.mat, tmp.act, tmp.time, tmp.msg);
                }
            }
        }
    ...
    if(initialize == 0)
    {
        tmp_act.resize(Nodes);
        pass.resize(Nodes);
        for (int i = 0; i < Nodes; i++)
        {
            pass[i].resize(Nodes);
            tmp_act[i].resize(Nodes);
        }
        initialize = 1;
    }
    ...
    {
        if(p->getHeader()->getPreID() == getNodeID()) //non-ack
        {
            int id = p2->getHeader()->getDstID();
            p2->getHeader()->setNexID(p2->getHeader()->getDstID());

            if(msg == "old")    //第一次更新(剛開始)
            {
                send_handler(p2);
                return;
            }
            if(msg == "start")  //更新記錄的node
            {
                pass[mat][id] = 2;    //working
                send_handler(p2);
                return;
            }
            if(nodetable[id].order[mat] == 0)   //可直接送出
            {            
                pass[mat][id] = 2;    //working
                send_handler(p2);
                return;
            ...
            {
                tmp_act[mat][id] = act;     //記錄尚不能更新的ctrl_packet
                pass[mat][id] = 1;    //waiting
            }
        }
        else //ack
        {
            if(msg == "old")
                return;
            int id = p2->getHeader()->getPreID();
            pass[mat][id] = 3;   //done
            //cout << id <<" is done.\n";
            for(int i = 0; i < Nodes; i++)
            {
                if(pass[mat][i] == 1 && tmp_act[mat][i] == id)  //下家是preID的node
                {
                    pass[mat][i] = 2;                    
                    ctrl_packet_event(Nodes, i, mat, tmp_act[mat][i], event::getCurTime(), "start");
                }
            }
            int ROP = 100000000;//Rate Of Progress
            for(int i = 0; i < Nodes; i++)
            {
                if((pass[mat][i] == 1 || pass[mat][i] == 2) && nodetable[i].order[mat] < ROP)
                    ROP = nodetable[i].order[mat];
            }
            
            for(int i = 0; i < Nodes; i++)
            {
                if(pass[mat][i] == 1 && nodetable[i].order[mat] == ROP)
                {
                    pass[mat][i] = 2;
                    ctrl_packet_event(Nodes, i, mat, tmp_act[mat][i], event::getCurTime(), "start");
                }
...
void SDN_switch::recv_handler(packet *p)
{
    ...
    if (p->type() == "SDN_stat_packet")
    {
        ...
        if(l4->getMsg() == "install")
        {
            add_phy_neighbor(Nodes);
            for(int i = 0; i < l4->getlinktable().size(); i++)
            {
                add_phy_neighbor(l4->getlinktable()[i].Node2);  //add neighbor
            }
        }
    ...
    { // the switch receives a packet from the controller
        ...
        nextID[mat] = act;          //update
        unsigned ctrl = p3->getHeader()->getSrcID();
        p3->getHeader()->setPreID(getNodeID());
        p3->getHeader()->setNexID(ctrl);
        p3->getHeader()->setDstID(ctrl);
        send_handler(p3);
        // string msg = l3->getMsg(); // get the msg
        // int msg_dst, msg_next;
        // istringstream Msg(msg);
        // Msg >> msg_dst >> msg_next; //從字串讀取2個資訊
        // nextID[msg_dst] = msg_next; //重新設定下家
        // string msg = l3->getMsg(); // get the msg
    }
...
int main() 
{
    ...
    node::node_generator::generate("SDN_controller", Nodes);
    
    for (int i = 0; i < Dsts; i++) // input dsts
    {
        int tmp = 0;
        cin >> tmp;
        dsts.push_back(tmp);
    ...
    vector<vector<Link>> linktable(Nodes);
    for (int i = 0; i < Links; i++) // input link
    {
        Link tmp;
        cin >> tmp.LinkID >> tmp.Node1 >> tmp.Node2 >> tmp.oldW >> tmp.newW;
        linktable[tmp.Node1].push_back(tmp);
        Link tmp2 = tmp;
        tmp2.Node1 = tmp.Node2;
        tmp2.Node2 = tmp.Node1;
        linktable[tmp2.Node1].push_back(tmp2);
    }

    for(int i = 0; i < Nodes; i++)
    {
        stat_packet_event(Nodes, i, linktable[i], InsTime, "install");
        stat_packet_event(Nodes, i, linktable[i], UpdTime, "update");
    }
    ...
    while(cin >> Time >> srcID >> dstID) // input data_packet_event
        data_packet_event(srcID, dstID, Time);        
